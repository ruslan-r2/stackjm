...
public class TestResourceAnswerController extends AbstractIntegrationTest {
    ...
    public void getAllAnswers() throws Exception {
        ...
        MvcResult mvcResult =
                this.mockMvc.perform(post(tokenURL).contentType(MediaType.APPLICATION_JSON).content(json))
                        .andDo(print())
        ...
        this.mockMvc.perform(get(URL, idWithoutAnswers).header("Authorization", token))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().string("[]"));
        //Не существующий ид вопроса
        this.mockMvc.perform(get(URL, idIncorrect).header("Authorization", token))
                .andDo(print())
