...
````
##Как с помощью конфигурации проекта задать свои значения БД

1. Переходим в **Edit Configurations...**
   
![](src/main/resources/static/images/editConfiguration.png)

2. Открываем вкладку **Environment** (1), далее в поле **Environment variables** (2) нажимаем **Shift+Enter** или значок
   списка.

![](src/main/resources/static/images/edit.PNG)

3. В открывшемся окне в поле **user environment variables** нажимаем **+**, будет добавлена пустая строка для записи 
   переменных среды.

![](src/main/resources/static/images/EnvVariables.png)

4. Добавляем информацию о своей базе данных, в поле **name** имя переменной, а в поле **value** значение
   (пример заполнения для БД PostgreSQL).
   
![](src/main/resources/static/images/EnvVariables2.png)

5. После создания переменных среды их можно использовать в файле **application.properties**. 
   <br/> Форма записи выглядит так: **${VAR_NAME}**
   
6. Для настройки приложения со своей базой данных необходимо использовать файл **application-dev.properties** 
   (профиль **dev**), в котором указываются значения переменной среды для своей базы данных.

![](src/main/resources/static/images/dev_for_db.png)

##Как в конфигурации проекта выбирать какой профиль загрузить

Для того чтобы загрузить необходимый профиль (например: **dev, local, test, prod**), в файле **application.properties**
необходимо раскомментировать строку **spring.profiles.active=dev** и после **=** указать, какой профиль необходимо загрузить.

![](src/main/resources/static/images/comment_dev_profile.PNG)

Или в конфигурации проекта **(Edit Configurations...)** в поле **Active profiles** указать требуемый профиль.

![](src/main/resources/static/images/activeProfilesField.png)

При запуске приложения можно увидеть, какой активирован профиль в данный момент.

![](src/main/resources/static/images/dev_profile.PNG)


# Авторизация через Postman для проверки API

1. Получить токен по адресу  http://localhost:8080/api/auth/token, передав в тело запроса (JSON) login и pass 

![](src/main/resources/static/images/postman1.PNG)

2. Для тестирования различных запросов с использованием полученного токена (не прописывая его в каждом запросе), создадим
коллекцию запросов и добавим среду (environment), добавляем новую переменную (1) и полученный токен (2), сохраняем. 

![](src/main/resources/static/images/postman2.png)

![](src/main/resources/static/images/postman3.png)

3. Переходим к созданной коллекции и во вкладке Authorization выбираем тип авторизации Bearer Token и в поле Token указываем имя своего созданного токена -> save.

![](src/main/resources/static/images/postman4.png)

4. Теперь можно создавать запросы на адрес своей API, в предварительно созданной коллекции и настроенной авторизации через Bearer Token 

![](src/main/resources/static/images/postman5.png)

# Oauth2 
После успешной авторизации через Oauth2 клиент, сгенерированный токен сохраниться в сookie с ключем "StackJM" и
выполнится редирект на /main. Так же дополнительно в хедер с ключем "OauthToken" сохранится json tokenResponseDTO

#Структура написания Тестов
1. # Названия всех Rest-контроллеров начинаются с Resource
   ```
   Пример: ResourceUserController
   ```
2. Тесты создаются согласно REST-контроллерам, в конце названия добавляется Test 
   ```
   Пример: если есть ResourceUserController, то создается тест
    ResourceUserControllerTest  и тестируются все api из контроллера
   ```
3. Все тесты наследуются от абстрактного класса **AbstractIntegrationTest**. От него же получаем **MockMvc** и токен для аутентификации из метода **getToken**
4. "**DataSet**"-ы создаем для каждого теста  в соответствующей директории с нижними разделяющими подчеркиваниями. Чужие DataSet-ы изменять нельзя!
   ```
   Пример: для ResourceUserController создаем директорию resource_user_controller
   в директории datasets
   ```   
5. ***ID*** сущностей в DataSet-ах начинаются с 100
6. Нельзя использовать аннотации транзакций в тестах
...
# Как использоватеь pagination.js
1. В заголовке страницы подключаем pagination.js. Пейджер реализован ввиде класса и использует темы из bootstrap.
    ```
    <HEAD>
    ...
...
2. После того, как подключили pagination.js, вызываем метод pager.setData(data, 'url', 'nameVar1', 'nameVar2').
   ```
   data - это JSON массив, который приходит с back-end.
   url - это часть урла до знака ? . например http://locaalhost:8080/api/user?currentPage=2&itemsOnPage=10
   в url мы передаём строку 'api/user' или 'http://locaalhost:8080/api/user'.
   nameVar1 - это имя переменной в url. Из примера выше это currentPage.
   nameVar2 - это имя переменной в url. Из примера выше это itemsOnPage.
   ```
   JSON массив data приходит с бэка ввиде
   ```
   { "currentPageNumber":1,
     "totalPageCount":12,
     "totalResultCount":59,
